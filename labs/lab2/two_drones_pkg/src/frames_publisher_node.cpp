...
class FramesPublisherNode{
  ...
  void onPublish(const ros::TimerEvent&){
    ...
    ros::Duration d = ros::Time::now() - startup_time;
    double time = d.toSec();
    ...
    AV1World.setOrigin(tf::Vector3(cos(time),sin(time),0));
    AV2World.setOrigin(tf::Vector3(sin(time),0,cos(2*time)));
    tf::Quaternion q;
    q.setRPY(0,0,time);
    // AV1World.setOrigin( tf::Vector3(2.0*sin(ros::Time::now().toSec()), 2.0*cos(ros::Time::now().toSec()), 0.0) );
    AV1World.setRotation(q);
    tf::Quaternion q2;
    q2.setRPY(0,0,0);
    AV2World.setRotation(q2);
    br.sendTransform(tf::StampedTransform(AV1World, ros::Time::now(), "world","av1"));
    br.sendTransform(tf::StampedTransform(AV2World, ros::Time::now(), "av1","av2"));
    ...
    // br.sendTransform(tf::StampedTransform(AV1World, ros::Time::now(),"world","av1broadcaster"));
    // *** FILL IN ***
