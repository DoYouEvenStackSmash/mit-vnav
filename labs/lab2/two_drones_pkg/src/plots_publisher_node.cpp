...
class PlotsPublisherNode{
 ...
 class TrajTrail{
   ...
   void update(){
     ...
     dest_frame = (dest_frame == "av2" ? "av1" : "av2");
     try {
      // ros::Duration(1.0).sleep();
      
      // listener.lookupTransform("av1","world",ros::Time::now(), transform);
      
      parent->tf_listener.lookupTransform(dest_frame,ref_frame,ros::Time::now(), transform);
